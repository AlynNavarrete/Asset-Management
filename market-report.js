document.addEventListener('DOMContentLoaded',()=>{
  ...
  const previewButton=button.cloneNode(true);previewButton.id='preview-market-report';previewButton.textContent='Ver reporte';button.before(previewButton);
  const generate=async(event)=>{
    const downloadOnly=event.currentTarget===button;
    const c=window.marketReportContext;
    if(!c||!window.jspdf?.jsPDF){alert('El generador PDF aún no está listo. Intenta descargar nuevamente.');return;}
    ...
    const fileName=`Reporte-Inteligencia-Detallado-${data.local}.pdf`,blob=pdf.output('blob');if(downloadOnly){window.rpDownloadFile(blob,fileName);if(toast){toast.textContent='Descarga del reporte iniciada.';setTimeout(()=>toast.classList.remove('visible'),2400);}return;}let dialog=document.getElementById('market-report-preview');if(!dialog){dialog=document.createElement('dialog');dialog.id='market-report-preview';document.body.appendChild(dialog);}dialog.innerHTML=`<section class="market-report-preview-panel"><header><div><small>Vista previa · formato carta</small><h2>Reporte de Inteligencia de Mercado · ${data.local}</h2></div><button type="button" data-close-market-report aria-label="Cerrar"><span class="material-symbols-outlined">close</span></button></header><div id="market-pdf-pages"></div><footer><button type="button" data-close-market-report>Cerrar</button><button type="button" data-save-market-report><span class="material-symbols-outlined">download</span> Descargar PDF</button></footer></section>`;dialog.querySelectorAll('[data-close-market-report]').forEach(item=>item.addEventListener('click',()=>dialog.close()));dialog.querySelector('[data-save-market-report]').addEventListener('click',()=>{window.rpDownloadFile(blob,fileName);});dialog.showModal();window.rpMountPdf(dialog.querySelector('#market-pdf-pages'),blob);dialog.addEventListener('close',()=>dialog.querySelector('#market-pdf-pages')?._pdfCleanup?.(),{once:true});
    if(toast){toast.textContent='Reporte listo para revisar.';setTimeout(()=>toast.classList.remove('visible'),2400);}
  };button.addEventListener('click',generate);previewButton.addEventListener('click',generate);
});
