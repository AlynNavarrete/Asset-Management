...
Descargas directas: `file-download.js` centraliza PDF, Excel, JSON, HTML, TXT y planos. Usa un enlace con nombre de archivo conectado al documento (o al diálogo activo), sin ventanas nuevas ni impresión, y mantiene el Blob disponible durante 120 segundos para el gestor de descargas. Inteligencia de Mercado distingue «Ver reporte» de «Descargar PDF». El navegador conserva el control de sus confirmaciones de guardado. Pruebas: `node tests/direct-downloads.cjs` y `node tests/mobile-pdf-exports.cjs`; `CHECK_ENGINE=webkit` permite ejecutarlas en el motor WebKit además de Chromium. No equivale a una prueba en un iPhone físico.

Las exportaciones del Directorio de Estaciones y Vacantes generan ahora libros OOXML `.xlsx` reales mediante ExcelJS 4.4.0 (carga local bajo demanda; licencia en `assets/vendor/exceljs/LICENSE`), reemplazando las tablas HTML con extensión `.xls`. Incluyen encabezado fijo, autofiltro, estilos, acentos, enlaces y valores numéricos para importes y superficies, conservando como texto IDs y entradas semejantes a fórmulas. `node tests/excel-export.cjs` verifica la descarga y vuelve a abrir el archivo para comprobar contenido y filas filtradas; `CHECK_ENGINE=webkit` ejecuta la prueba en WebKit móvil.
